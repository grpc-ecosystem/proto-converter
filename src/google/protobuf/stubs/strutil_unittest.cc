...
TEST(Base64, EscapeAndUnescape) {
  ...
  for (int i = 0; i < sizeof(base64_tests) / sizeof(base64_tests[0]); ++i) {
    ...
    absl::string_view plaintext(base64_tests[i].plaintext,
    ...
    decode_str.clear();
    EXPECT_TRUE(Base64Unescape(absl::string_view(encode_buffer, cipher_length),
                               &decode_str));
    ...
    if (first_equals) {
      ...
      *first_equals = '\0';
      EXPECT_TRUE(Base64Unescape(
          absl::string_view(encode_buffer, first_equals - encode_buffer),
          &decoded2));
      EXPECT_EQ(decoded.size(), base64_tests[i].plain_length);
      EXPECT_EQ_ARRAY(decoded.size(), decoded, base64_tests[i].plaintext, i);
      ...
      *(first_equals + 1) = '\0';
      EXPECT_TRUE(Base64Unescape(
          absl::string_view(encode_buffer, first_equals - encode_buffer + 1),
          &decoded2));
      EXPECT_EQ(decoded.size(), base64_tests[i].plain_length);
      EXPECT_EQ_ARRAY(decoded.size(), decoded, base64_tests[i].plaintext, i);
      ...
      *(first_equals + 1) = '\0';
      ...
      decoded2.assign("this junk should be ignored");
      EXPECT_TRUE(
          Base64Unescape(absl::string_view(encode_buffer, len), &decoded2));
      EXPECT_EQ(decoded.size(), base64_tests[i].plain_length);
      EXPECT_EQ_ARRAY(decoded.size(), decoded, base64_tests[i].plaintext, i);
    ...
    encode_length =
        WebSafeBase64Escape(unsigned_plaintext, base64_tests[i].plain_length,
                            encode_buffer, sizeof(encode_buffer), true);
    //    Is it of the expected length?
    EXPECT_EQ(encode_length, cipher_length);
    EXPECT_EQ(CalculateBase64EscapedLen(base64_tests[i].plain_length, true),
              encode_length);
    ...
    EXPECT_TRUE(WebSafeBase64Unescape(
        absl::string_view(encode_buffer, cipher_length), &decoded));
    EXPECT_EQ(decoded.size(), base64_tests[i].plain_length);
    EXPECT_EQ_ARRAY(decoded.size(), decoded, base64_tests[i].plaintext, i);
    ...
    encode_length =
        WebSafeBase64Escape(unsigned_plaintext, base64_tests[i].plain_length,
                            encode_buffer, sizeof(encode_buffer), false);
    //    Is it of the expected length?
    EXPECT_EQ(encode_length, cipher_length);
    EXPECT_EQ(CalculateBase64EscapedLen(base64_tests[i].plain_length, false),
              encode_length);
  ...
  for (int i = 0; i < sizeof(base64_strings) / sizeof(base64_strings[0]); ++i) {
    const unsigned char* unsigned_plaintext =
    ...
    int encode_length = WebSafeBase64Escape(unsigned_plaintext, plain_length,
                                            &buffer[0], buffer.size(), false);
    EXPECT_EQ(cipher_length, encode_length);
    EXPECT_EQ(CalculateBase64EscapedLen(plain_length, false), encode_length);
  ...
  // Verify the behavior when decoding bad data
  {
    ...
    std::string buf;
    EXPECT_FALSE(Base64Unescape(absl::string_view(bad_data), &buf));
