...
const google::protobuf::Type* DefaultValueObjectWriter::Node::GetMapValueType(
  ...
  for (int i = 0; i < found_type.fields_size(); ++i) {
    ...
    if (!sub_type.ok()) {
      ABSL_LOG(WARNING) << "Cannot resolve type '" << sub_field.type_url()
                        << "'.";
    } else {
  ...
  for (int i = 0; i < type_->fields_size(); ++i) {
    ...
    if (field.kind() == google::protobuf::Field::TYPE_MESSAGE) {
      ...
      if (!found_result.ok()) {
        ...
        ABSL_LOG(WARNING) << "Cannot resolve type '" << field.type_url()
                          << "'.";
      } else {
...
DataPiece DefaultValueObjectWriter::FindEnumDefault(
  ...
  if (!enum_type) {
    ABSL_LOG(WARNING) << "Could not find enum with type '" << field.type_url()
                      << "'";
  ...
  if (!field.default_value().empty()) {
    if (!use_ints_for_enums) {
      ...
      ABSL_LOG(WARNING) << "Could not find enum value '"
                        << enum_default_value_name << "' with type '"
                        << field.type_url() << "'";
      return DataPiece::NullData();
    }
  }
...
void DefaultValueObjectWriter::RenderDataPiece(absl::string_view name,
  ...
  if (current_->type() != nullptr && current_->type()->name() == kAnyType &&
    ...
    if (data_string.ok()) {
      ...
      if (!found_type.ok()) {
        ABSL_LOG(WARNING) << "Failed to resolve type '" << string_value << "'.";
      } else {
